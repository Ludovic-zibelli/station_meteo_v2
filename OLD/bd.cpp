...
bool updateStationDirect(
  ...
  if (rc != SQLITE_OK) {
    Serial.printf("[SQLite] prepare station_direct failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] prepare station_direct failed: (%d) %s\n", rc, sqlite3_errmsg(db));
  ...
  if (rc != SQLITE_DONE) {
    Serial.printf("[SQLite] step station_direct failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] step station_direct failed: (%d) %s\n", rc, sqlite3_errmsg(db));
...
bool updateAnemometre(int id, float anemometre) {
  ...
  int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
  if (rc != SQLITE_OK) {
    Serial.printf("[SQLite] prepare updateAnemometre failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] prepare updateAnemometre failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    if (g_dbMutex) xSemaphoreGive(g_dbMutex);
    return false;
  }
  ...
  rc = sqlite3_step(stmt);
  if (rc != SQLITE_DONE) {
    Serial.printf("[SQLite] step updateAnemometre failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] step updateAnemometre failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    sqlite3_finalize(stmt);
    if (g_dbMutex) xSemaphoreGive(g_dbMutex);
    return false;
  }
...
bool updateTensions(int id, float tension_batterie, float tension_solaire) {
  ...
  int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
  if (rc != SQLITE_OK) {
    Serial.printf("[SQLite] prepare tensions failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] prepare tensions failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    if (g_dbMutex) xSemaphoreGive(g_dbMutex);
    return false;
  }
  ...
  rc = sqlite3_step(stmt);
  if (rc != SQLITE_DONE) {
    Serial.printf("[SQLite] step tensions failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] step tensions failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    sqlite3_finalize(stmt);
    if (g_dbMutex) xSemaphoreGive(g_dbMutex);
    return false;
  }
...
bool updateEtatCapteurs(int id,
  ...
  int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
  if (rc != SQLITE_OK) {
    Serial.printf("[SQLite] prepare etatcapteurs failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] prepare etatcapteurs failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    if (g_dbMutex) xSemaphoreGive(g_dbMutex);
    return false;
  }
  ...
  rc = sqlite3_step(stmt);
  if (rc != SQLITE_DONE) {
    Serial.printf("[SQLite] step etatcapteurs failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    app_logf("[SQLite] step etatcapteurs failed: (%d) %s\n", rc, sqlite3_errmsg(db));
    sqlite3_finalize(stmt);
    if (g_dbMutex) xSemaphoreGive(g_dbMutex);
    return false;
  }
...
bool updateActivationApiInDB(int id, int activation_envoi_api) {
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) {
        ...
        app_logf("Erreur prepare (updateActivationApiInDB): %s\n", sqlite3_errmsg(db));
        return false;
    }
    ...
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        ...
        app_logf("Erreur step (updateActivationApiInDB): %s\n", sqlite3_errmsg(db));
        sqlite3_finalize(stmt);
        if (g_dbMutex) xSemaphoreGive(g_dbMutex);
        return false;
    }
