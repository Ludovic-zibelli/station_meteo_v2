...
document.getElementById('btnLoadOfs').onclick = async ()=>{
  ...
  const j = await r.json();
  document.getElementById('ofs_t_bmp_cfg').value   = j.t_bmp ?? 0;
  document.getElementById('ofs_t_dht_cfg').value   = j.t_dht ?? 0;
  document.getElementById('ofs_h_dht_cfg').value   = j.h_dht ?? 0;
  document.getElementById('ofs_press_cfg').value   = j.press ?? 0;
  document.getElementById('ofs_wind_cfg').value    = j.wind ?? 0;
  document.getElementById('ofs_t_sht40_cfg').value = j.t_sht40 ?? 0;
};
document.getElementById('btnSaveOfs').onclick = async ()=>{
  const fd = new FormData();
  fd.append('t_bmp',   document.getElementById('ofs_t_bmp_cfg').value || 0);
  fd.append('t_dht',   document.getElementById('ofs_t_dht_cfg').value || 0);
  fd.append('h_dht',   document.getElementById('ofs_h_dht_cfg').value || 0);
  fd.append('press',   document.getElementById('ofs_press_cfg').value || 0);
  fd.append('wind',    document.getElementById('ofs_wind_cfg').value || 0);
  fd.append('t_sht40', document.getElementById('ofs_t_sht40_cfg').value || 0);
